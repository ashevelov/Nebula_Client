...
public partial class SGT_ThrusterController : SGT_MonoBehaviourUnique<SGT_ThrusterController>
{
	...
	public void ThrusterLinearBurn(Vector3 direction, float throttle, UnityEngine.Space space)
	{
		if (thrusters != null)
		{
			if (throttle < 0)
	...
	public void ThrusterAngularBurn(Vector3 axis, float throttle, UnityEngine.Space space)
	{
		if (thrusters != null)
		{
			if (throttle < 0)
			...
			foreach (var thruster in thrusters)
			{
				...
				var centre       = SGT_Helper.ClosestPointToLineB(GetComponent<Rigidbody>().worldCenterOfMass, axis, thruster.transform.position);
