...
public class MapController : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        curentSS = _starSystems.Find((s) => { return s._id == DataResources.Instance.ZoneForId(G.Game.Engine.GameData.World.Name).Scene(); });
            ...
            {
                {
                    ConfirmationDialog.Setup(Nebula.Resources.StringCache.Get("JUMP"), () =>
                    {
                        ...
                        if (game.Engine.GameData.World.Name == worldNameId)
                        {
                            Debug.Log("you already in this world");
                        }
                        else
                        {
                            NetworkGame.ChangeWorld(worldNameId);
                        }
