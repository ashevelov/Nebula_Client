...
namespace Nebula {
    public abstract class RPCOperationReturnStrategy : OperationReturnStrategy
    {
        ...
        protected string ErrorMessageId(OperationResponse response) {
            return Result(response).GetValueOrDefault<string>(GenericEventProps.error_message_id);
        }
        ...
        protected void PrintRPCError(OperationResponse response)
        {
            if(Status(response) == ACTION_RESULT.FAIL)
            {
                ...
                G.Game.Chat.PastLocalMessage(errorMessage);
            }
        }
        ...
        public  void UpdateInventorySourceView(OperationResponse response) {
            if (Status(response) == ACTION_RESULT.SUCCESS) {
                ...
                if(retHash != null ) {
                    ...
                    string containerId = retHash.GetValue<string>(GenericEventProps.container_id, string.Empty);
                    byte containerType = retHash.GetValue<byte>(GenericEventProps.container_type, ItemType.Ghost.toByte());
                    string containerContentItemId = retHash.GetValue<string>(GenericEventProps.container_item_id, string.Empty);
