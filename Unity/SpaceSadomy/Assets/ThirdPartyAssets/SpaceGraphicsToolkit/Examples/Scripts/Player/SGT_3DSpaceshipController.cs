...
public class SGT_3DSpaceshipController : SGT_MonoBehaviour
{
	...
	public void Update()
	{
		if (ThrusterController != null)
		{
			...
			// Move Forwards
            ThrusterController.ThrusterLinearBurn(Vector3.forward, Input.GetAxis("Jump"), UnityEngine.Space.Self);
			
			// Yaw
            ThrusterController.ThrusterAngularBurn(Vector3.up, Input.GetAxis("Horizontal"), UnityEngine.Space.Self);
			
			// Pitch
            ThrusterController.ThrusterAngularBurn(Vector3.right, Input.GetAxis("Vertical"), UnityEngine.Space.Self);
