...
public class SGT_2DSpaceshipController : SGT_MonoBehaviour
{
	...
	public void Update()
	{
		if (ThrusterController != null)
		{
			...
			ThrusterController.ThrusterLinearBurn(Vector3.forward, Input.GetAxis("Vertical"), UnityEngine.Space.Self);
			
			// Move Right/Left
			//ThrusterController.ThrusterLinearBurn(Vector3.right, Input.GetAxis("Some Other Axis"), Space.Self);
			
			// Rotate Right/Left
            ThrusterController.ThrusterAngularBurn(rotationAxis, Input.GetAxis("Horizontal"), UnityEngine.Space.Self);
		}
	}
}