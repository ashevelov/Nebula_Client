namespace Nebula.Mmo.Games.Strategies.Operations.SelectCharacter {
    ...
    public class GetMailsOperation : BaseOperationHandler {
        public override void Handle(BaseGame game, OperationResponse response) {
            ...
            Hashtable mailHash = response.Parameters[(byte)ParameterCode.MailBox] as Hashtable;
            if(mailHash == null) {
                Debug.LogError(string.Format("mail hash is null"));
                return;
            }

            game.Engine.GameData.mailBox.ParseInfo(mailHash);
        }
    }
}
